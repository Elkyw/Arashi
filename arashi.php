...
function update()
    {
        echo "\n\e[91m\e[1m[+] Arashi UPDATE UTILITY [+]\nUpdate in progress, please wait...\n\n$cln";
        system("git fetch origin && git reset --hard origin/master && git clean -f -d");
        echo $bold . $fgreen . "[i] Job finished successfully! Please Restart Arashi \n" . $cln;
        exit;
    }
    ...
    if (!in_array($scan, array(
    ...
    else
      {
        ...
        elseif ($scan == "8")
          {
            ...
            if (count($array) > 0)
              {
                userinput("Do You Want Arashi To Detect CMS Of The Sites? [Y/N]");
            ...
            if (strpos($srccd, "wp-content") !== false)
              {
                ...
                if ($wp_version != "")
                  ...
                  {
                    $reallink = $ipsl . $ip;
                    echo $red . "Failed \n\n[!] Arashi could not determine the WordPress version of the target!";
                    echo "\n\n";
                    echo $bold . $yellow . "[*] Scanning Complete. Press Enter To Continue OR CTRL + C To Stop\n\n";
                    trim(fgets(STDIN, 1024));
                    goto scanlist;
        ...
        elseif ($scan == 'U' || $scan == 'u')
          {
            echo "\n\n" . $bold . $yellow . "-[ Arashi Update Corner]-\n\n" . $cln;
            echo $bold . "[i] Fetching Stuffs .... \n" . $cln;
            $latestversion = readcontents("https://raw.githubusercontent.com/Elkyw/Arashi/master/version.txt");
            echo $bold . $blue . "[C] Current Version: " . $arversion . $cln;
            echo "\n" . $bold . $lblue . "[L] Latest Version:  " . $latestversion . $cln;
            if ($latestversion > $arversion)
              {
                askupdate:
            ...
            elseif ($arversion == $latestversion)
              {
                echo $bold . $fgreen . "\n[i] You are already running the latest version of Arashi. \n\n" . $cln;
                goto scanlist;
              }
            else
              {
